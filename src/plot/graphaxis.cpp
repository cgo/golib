...
namespace goPlot
{
     ...
     */
    GraphAxis::GraphAxis (int index, double l, double u)
        : Object2DPoints (2),
        myLower (l), 
        myUpper (u),
        myIndex (index),
     ...
     */
    real GraphAxis::x () const { return myPosX; }
    /** 
     ...
     */
    real GraphAxis::y () const { return myPosY; }
    /** 
     ...
     */
    void GraphAxis::setPosition (real x, real y) { myPosX = x; myPosY = y; }
     ...
     */
    const char* GraphAxis::ticsFont () const { return myTicsFont.c_str (); }
    /** 
     ...
     */
    void GraphAxis::setTicsFont (const char* f) 
    {
        myTicsFont = f; 
        this->updateTicsText ();
    }
     ...
     */
    bool GraphAxis::visible () const { return (myConf & VISIBLE) != 0; }
     ...
     */
    void GraphAxis::setVisible (bool v) 
    {
        if (v)
     ...
     */
    void GraphAxis::configure (int c)
    {
        switch (c & POS_MASK)
        {
            case RIGHT: 
     ...
     */
    int GraphAxis::configuration () const { return myConf; }
     ...
     */
    void GraphAxis::enableTics (bool f) 
    {
        if (f)
            myConf = myConf | TICS;
        else
     ...
     * If false, the opposite will happen.
     */
    void GraphAxis::enableTicsText (bool f) 
    {
        if (f)
     ...
     */
    void GraphAxis::drawTics (cairo_t* cr)
    {
        cairo_save (cr);
     ...
     * @return Axis index
     */
    int    GraphAxis::index () const { return myIndex; };
     ...
     */
    void   GraphAxis::setTics (const std::vector<real>& t) 
    { 
        myTics = t; 
        this->updateTicsText ();
    }
     ...
     */
    void GraphAxis::setTics (real start, real step, real end) 
    {
     ...
     */
    void GraphAxis::setTics (int count)
    {
        real l = this->lower ();
        real u = this->upper ();
     ...
     */
    void GraphAxis::getTicsTextPathExtents (double& x1, double& y1, double& x2, double& y2)
    {
        cairo_t *cr = this->context ();
        if (!cr)
            return;
     ...
     * @param y2 Border at bottom
     */
    void GraphAxis::getBorderHint (double w, double h, double& x1, double& y1, double& x2, double& y2)
    {
        if (!this->visible ())
        {
            x1 = 0.0;
